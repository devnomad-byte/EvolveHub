...
const displayIcons = computed(() => {
  ...
  if (selectedCategoryId.value === 'uncategorized') {
    return icons.value.filter(i => !i.categoryId || i.categoryId === 0)
  }
  if (selectedCategoryId.value) {
    return icons.value.filter(i => i.categoryId === selectedCategoryId.value)
  }
  return icons.value
})

const uncategorizedCount = computed(() => {
  return icons.value.filter(i => !i.categoryId || i.categoryId === 0).length
})
...
function getEmoji(iconName: string): string {
  const map: Record<string, string> = {
    MessageSquare: '💬', BookOpen: '📚', Bot: '🤖', Users: '👥', Building: '🏢',
    Wrench: '⚙️', Zap: '⚡', Settings: '⚙️', Monitor: '🖥️', Shield: '🛡️',
    FileText: '📄', Server: '🖥️', MessageSquareText: '💬', BarChart3: '📊',
    HardDrive: '💾', Clock: '⏰', Grid: '📦', Star: '⭐', Folder: '📁',
    Database: '🗄️', Cloud: '☁️', Key: '🔑', Scan: '🔍', Cat: '🐱', LayoutGrid: '📦'
  }
  return map[iconName] || '📦'
}
